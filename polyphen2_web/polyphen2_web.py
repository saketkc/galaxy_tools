...
## Genome assembly version used for chromosome coordinates of the SNPs in user input
UCSCDB = ["hg19", "hg18"]
...
class Polyphen2Web:
    ...
    def make_request( self ):
        ...
        contents = open(self.file_location,"r").read().replace("\t"," ").replace("::::::::::::::","")
                ...
                if running_jobs_table:
                    ...
                    if len(rows)==1:
                        ...
                        if len(hrefs)>=3:
                            ...
                            short_txt = result_url+path + "-short.txt"#short_txt
                            url_dict = {
                                            "full_file":full_txt,
                                            "snps_file":snps_txt,
                                            "log_file":log_txt,
...
def main(args):
    ...
    parser.add_argument('-u', '--ucscdb',dest='ucscdb', choices=UCSCDB, required=True, type=str)
    parser.add_argument('-m', '--model', dest='modelname', choices=MODELNAME, required=True, type=str)
    parser.add_argument('-fl', '--filter', '--snpfilter', dest='snpfilter', choices=SNPFILTER.keys(), required=True, type=str)
    parser.add_argument('-i', '--input', dest='input', nargs='?', required=True, type=str, default=sys.stdin)
    parser.add_argument('-e', '--email', dest='email', required=False, default=None)

    parser.add_argument('--log', dest='log_file', required=True, default=None, type=str)
    parser.add_argument('--short', dest='short_file', required=True, default=None, type=str)
    parser.add_argument('--full', dest='full_file', required=True, default=None, type=str)
    parser.add_argument('--snp', dest='snps_file', required=True, default=None, type=str)
    parser.add_argument('--function', dest='snpfunction', required=True, type=str)
    args_s = vars(parser.parse_args(args))
    polyphen2_web = Polyphen2Web(ucscdb=args_s["ucscdb"], model_name=args_s["modelname"], snp_filter=args_s["snpfilter"], snp_function=args_s["snpfunction"], file_location=args_s["input"], email=args_s["email"])

    sid = polyphen2_web.make_request()
    if not sid:
        stop_err("Something went wrong! The tracking id could not be retrieved.")
